...
const ProductConfigPanel = ({ activeProduct, isPanelOpen, setIsPanelOpen, addCartItem, productTotalQtyInCart }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (productData.status && productData.data) {
          ...
          setBundleShirtColor(productData.data.options && productData.data.options.length > 0 ? productData.data.options[0].name : null);
  ...
  console.log("Rendering ProductConfigPanel with activeProduct:", activeProduct);

  const isBundle = activeProduct?.product_type.toLowerCase().includes("bundle") ?? false
 ...
 useEffect(() => {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setIsPanelOpen(false)}></div>
      <div
      ...
      >
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white z-10">
          <div>
            <h3 className="font-bold text-xl text-gray-900">{activeProduct.title}</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          <div className="bg-green-50 border border-green-200 p-3 rounded-xl flex items-center gap-3 animate-in fade-in">
            ...
            <div className="text-sm font-bold text-green-800 leading-tight">
              Mengenrabatte werden für alle Farben dieses Produkts zusammengezählt!
            </div>
          </div>
          ...
          {isBundle && bundleStep === 2 ? (
            <div className="animate-in slide-in-from-right">
              ...
              <h4 className="font-bold text-lg mb-4 text-gray-900">Größen für Zusatzartikel</h4>
              ...
              <div className="mb-6">
                <h4 className="font-bold text-sm text-gray-500 uppercase mb-3">Farbe (2. Teil)</h4>
              ...
              <div className="grid grid-cols-3 gap-3">
                {availableSizes.map(sizeObj => {
                  const currentBundleQty = Object.values(bundleShirtSize).reduce((a, b) => a + b, 0)
                  ...
                  return (
                    ...
                    >
                      <span className="text-xs font-bold text-gray-400 mb-1">{sizeObj.value}</span>
                      <input
                        type="number"
                        min="0"
                        ...
                        onChange={e => {
                          ...
                          if (currentBundleQty - val + newVal <= localQty) {
                            setBundleShirtSize({ ...bundleShirtSize, [sizeObj.value]: newVal })
                          }
                        }}
                      />
                    </div>
            ...
            <>
              <div>
                <h4 className="font-bold text-sm text-gray-500 uppercase mb-3">Farbe {isBundle ? "(Hoodie)" : ""}</h4>
                <div className="flex flex-wrap gap-3">
                  {productOptions.map(option => (
                    option.name.startsWith("#") ? 
                    (<button
                      ...
                      onClick={() => setBundleShirtColor(option.name)}
                      className={`w-12 h-12 rounded-full shadow-sm ring-offset-2 transition-all ${
                        bundleShirtColor === option.name ? "ring-2 ring-orange-600 scale-110" : "ring-1 ring-gray-200"
                      }`}
                    />):
                    (
                    <button
              ...
              <div>
                <h4 className="font-bold text-sm text-gray-500 uppercase mb-3">Größen {isBundle ? "(Hoodie)" : ""}</h4>
                <div className="grid grid-cols-3 gap-3">
                  {availableSizes.map(sizeObj => (
                    <div
                      key={sizeObj.id}
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50">
          {isBundle && bundleStep === 1 ? (
            <Button onClick={() => setBundleStep(2)} className="w-full" disabled={localQty === 0}>
              Größen für 2. Teil eintragen
            </Button>
          ) : (
            <Button
              onClick={() => {
                const unitPrice = priceList[projectedTierIndex]?.price || priceList[0]?.price
                addCartItem(
                  activeProduct,
                  selectedOption?.name,
                  unitPrice,
                  localSizes,
                  isBundle ? { hoodie: localSizes, shirt: bundleShirtSize } : null
                )
              }}
              className="w-full"
              disabled={localQty === 0 || !selectedOption || (isBundle && bundleStep === 2 && Object.values(bundleShirtSize).reduce((a, b) => a + b, 0) !== localQty)}
            >
              Hinzufügen
            </Button>
          )}
